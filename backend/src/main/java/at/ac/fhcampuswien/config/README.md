...
3. **`BearerTokenAuthenticationFilter`** — Spring's OAuth2 Resource Server filter. Reads `Authorization: Bearer …`, verifies the JWT, and sets the `SecurityContext`. See `session/README.md`.
...
- `session/README.md` — the JWT filter this config registers.
- `auth/README.md` — endpoint catalog and password handling.
