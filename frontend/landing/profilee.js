// ============================================================================
// 1. DYNAMIC DATA LOADING & CORE LOGIC
// ============================================================================
...
async function loadProfile() {
    try {
        ...
        // Sichtbarkeit des Melde- und Edit-Buttons steuern
        ...
        const chatButton = document.getElementById("profile-message-button");

        if (targetUserId && targetUserId !== localStorage.getItem("designer_jobs_userId")) {
            // Es ist ein FREMDES Profil
            if (reportButton) reportButton.style.display = "block";  // Melden erlauben
            if (chatButton) chatButton.style.display = "block";  //
            if (editButton) editButton.style.display = "none";      // Bearbeiten verbieten
        } else {
            // Es ist das EIGENE Profil
            if (reportButton) reportButton.style.display = "none";   // Sich selbst melden verbieten
            chatButton.textContent = "View Messages";
            if (editButton) editButton.style.display = "block";     // Bearbeiten erlauben
        }
        ...
        const bioDetailElement = document.getElementById("profile-bio-detail");
        if (bioDetailElement) {
            bioDetailElement.textContent = user.bio && user.bio.trim() !== ""
                ? user.bio
                : "No biography provided added yet.";
        }

        const locationElement = document.getElementById("profile-location-detail");
        if (locationElement) {
            let locString = "";
            if (user.city) locString += user.city;
            if (user.country) locString += (locString ? ", " : "") + user.country;
            locationElement.textContent = locString || "Not specified";
        }

        document.getElementById("profile-design-type").textContent = user.designType || "-";
        renderSkills(user.skills);

        const hourlyRateElement = document.getElementById("profile-hourly-rate");
        if (hourlyRateElement) {
            if (user.hourlyMin || user.hourlyMax) {
                hourlyRateElement.textContent = `${user.hourlyMin || 0}€ - ${user.hourlyMax || 0}€ / hr`;
            } else {
                hourlyRateElement.textContent = "-";
            }
        }

        updateSocialLink("profile-link-twitter", user.twitter);
        updateSocialLink("profile-link-linkedin", user.linkedin);
        updateSocialLink("profile-link-instagram", user.instagram);

        // Portfolio-Sichtbarkeit anhand der Rolle steuern
        const portfolioSection = document.getElementById("portfolio-section");
        const designerInfoSection = document.getElementById("designer-info-section");

        if (role === "DESIGNER") {
            if (portfolioSection) portfolioSection.style.display = "block";
            if (designerInfoSection) designerInfoSection.style.display = "block";
            loadDesignInspiration();
        } else {
            if (portfolioSection) portfolioSection.style.display = "none";
            if (designerInfoSection) designerInfoSection.style.display = "none";
        }
...
/**
 * Lädt Beispielbilder aus der Pexels-API (Wird jetzt dynamisch für Designer getriggert)
 */
async function loadDesignInspiration() {
    ...
    try {
        const response = await fetch("http://localhost:8080/api/design-inspiration?query=graphic%20design");
...
function updateSocialLink(elementId, url) {
    ...
    if (linkElement) {
        if (url && url.trim() !== "") {
            // Sicherstellen, dass die URL mit http anfängt, falls der User es beim Editieren vergessen hat
            const absoluteUrl = url.startsWith("http://") || url.startsWith("https://") ? url : `https://${url}`;
            linkElement.href = absoluteUrl;
            linkElement.textContent = url;
            linkElement.style.pointerEvents = "auto";
            linkElement.classList.remove("text-secondary", "pe-none");
        } else {
            linkElement.removeAttribute("href");
            linkElement.textContent = "Not specified";
            linkElement.classList.add("text-secondary", "pe-none"); // Link ausgrauen und unklickbar machen
        }
    }
}
...
if (profileReportButton) {
    profileReportButton.addEventListener("click", async () => {
        ...
        try {
            const response = await fetch(`/moderation/users/${targetUserId}/report`, {
                method: "POST",
                headers: {
...
// Startet den gesamten Profil-Ladevorgang beim Aufruf
loadProfile();